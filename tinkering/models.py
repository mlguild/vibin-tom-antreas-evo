...
class SimpleResNet(nn.Module):
    def __init__(
        self, block, num_blocks_per_stage, num_classes=10, in_channels=1
    ):
        ...
        self.layer1 = self._make_layer(
            block, 64, num_blocks_per_stage[0], stride=1
        )
        self.layer2 = self._make_layer(
            block, 128, num_blocks_per_stage[1], stride=2
        )
        self.layer3 = self._make_layer(
            block, 256, num_blocks_per_stage[2], stride=2
        )
        self.layer4 = self._make_layer(
            block, 512, num_blocks_per_stage[3], stride=2
        )
